...
### ~~conversation 학습 오염~~ → **2026-06-14 해결 확정**
- 2026-06-13 9차 풀 플로우 테스트(478초, 9/9 성공)의 결과 PNG 3장(01 hero / 05 farm scene / 09 outro) 직접 확인 → **모두 정확한 사과 결과**
- 결정적 요인: 매 탭에 사장님 제품 히어로 사진을 `attachImageToInput`으로 첨부 → ChatGPT가 텍스트보다 첨부 이미지를 압도적으로 우선 → 그 대화의 과거 컨텍스트(Causone 박스/카메라 펜던트)에 끌려가지 않음
- 사장님 직관 ("어차피 새로 말 거는 건데") 실측으로 검증됨
- 빈 대화 9개 만들기 작업 불필요 — 기존 대화 재활용 OK
- 영구 규칙으로 승격: [feedback_product_hero_reference.md](./feedback_product_hero_reference.md)
...
1. ~~빈 대화 자동 생성 모드~~ — 학습 오염 해결 확정으로 불필요
2. ~~대화 선택 휴리스틱~~ — 동상
...
6. **디자인 보완** — 사장님 평가 "기능적으로 완벽, 디자인은 별도 보완 계속" (2026-06-13)
